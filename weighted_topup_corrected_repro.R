#!/usr/bin/env Rscript
# =============================================================================
# WEIGHTED SEED TOP-UP -- ONE fit per invocation, at ANY of the 7 Kss grid points,
...
#
# Usage:  Rscript weighted_topup_corrected_repro.R <grid_point_index 1-7> <seed>
#
# Question this answers: does the repro profile's jagged, under-resolved shape
# at 4 grid points smooth out with more seeds (flat profile is real) or persist
# (shape genuinely unstable)?
#
# Does NOT touch multistart_results_corrected_full.csv, multistart_results_corrected_pilot.csv, convergence_test_N80_Kss1263.csv, or multistart_results_corrected_repro.csv (an unrelated, unused file from an earlier, different worker).
# APPENDS to gate_test_N30_Kss1263_repro.csv -- the file that already holds the
# full 35-row repro profile (7 grid points x 5 seeds: 42,123,999,7,2024) -- so
# the 12 new fits (fresh seeds 1, 2025, 314, 777, at 4 under-resolved grid
# points) combine with the existing rows into a topped-up profile. Logs to
# weighted_topup_corrected_repro_logs/.
#
...
if (length(args) != 2) {
  stop("Usage: Rscript weighted_topup_corrected_repro.R <grid_point_index 1-7> <seed>")
}
...
cat(sprintf("=== WEIGHTED TOP-UP  grid_point=%d/%d  Kss=%.5f  seed=%d  nBurn=150 nEm=200  (%d rows, %d subjects, CORRECTED model) ===\n",
