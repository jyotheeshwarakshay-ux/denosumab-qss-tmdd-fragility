#!/usr/bin/env Rscript
# =============================================================================
# ONE (grid_point, seed) FIT -- pilot worker for the CORRECTED Kss profile.
#
# Usage:  Rscript multistart_profile_Kss_corrected_pilot.R <grid_point_index 1-7> <seed>
#
...
#   - Reads simulated_denosumab_data_v2_corrected_pilot.csv (fresh 30-subject
#     data from the corrected model), NOT simulated_denosumab_data_v2.csv (the
#     old superseded, buggy-model dataset).
#   - Writes to multistart_results_corrected_pilot.csv, a NEW file -- does not
...
# nEm=200) on the fresh 30-subject pilot dataset, and appends ONE row to
# multistart_results_corrected_pilot.csv:
...
if (length(args) != 2) {
  stop("Usage: Rscript multistart_profile_Kss_corrected_pilot.R <grid_point_index 1-7> <seed>")
}
...
results_csv <- file.path(repo, "multistart_results_corrected_pilot.csv")
...
if (!file.exists(data_csv)) {
  stop("Fresh pilot dataset not found: ", data_csv,
       " -- run generate_corrected_pilot_data.R first.")
}
...
cat(sprintf("=== PILOT grid_point=%d/%d  Kss=%.5f  seed=%d  (%d rows, %d subjects, CORRECTED model) ===\n",
