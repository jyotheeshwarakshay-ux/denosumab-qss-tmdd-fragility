#!/usr/bin/env Rscript
# =============================================================================
# REPRODUCIBILITY GATE -- ONE fit per invocation, at Kss FIXED = 1.263,
...
#
# Usage:  Rscript gate_test_N30_Kss1263_repro.R <seed>
#
...
#
# Does NOT touch multistart_results_corrected_full.csv, multistart_results_corrected_pilot.csv, or convergence_test_N80_Kss1263.csv.
# Writes ONLY to gate_test_N30_Kss1263_repro.csv (new file) and
# gate_test_N30_Kss1263_repro_logs/ (new directory).
#
...
if (length(args) != 1) {
  stop("Usage: Rscript gate_test_N30_Kss1263_repro.R <seed>")
}
...
if (file.exists(results_csv)) {
  ...
  if (!is.null(existing) && nrow(existing) > 0 && "seed" %in% names(existing) &&
      any(existing$seed == seed)) {
...
if (!file.exists(data_csv)) {
  stop("Fresh full dataset not found: ", data_csv,
       " -- expected the fresh repro dataset -- run generate_corrected_repro_data.R first.")
}
...
cat(sprintf("=== REPRO GATE  Kss=%.5f (FIXED)  seed=%d  nBurn=150 nEm=200  (%d rows, %d subjects, CORRECTED model) ===\n",
            kss_value, seed, nrow(pk_prof), length(unique(pk_prof$ID))))
